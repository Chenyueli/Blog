...
预处理：创建一个词法环境（LexicalEnvironment，简写为LE），扫描JS中的**用声明的方式声明的函数**(（不是函数表达式）)，**用var定义的变量**并将它们加到预处理阶段的词法环境中去。

**处理函数声明有冲突时，会覆盖；处理变量声明有冲突时，会忽略。**
...
#### apply用于函数引用类型参数传递
apply可将一个数组转换为一个参数列表。适用于目标函数只需要n个参数列表,而不接收一个数组的形式。
